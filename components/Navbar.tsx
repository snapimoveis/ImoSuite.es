    ...
    <nav className="fixed top-0 left-0 right-0 z-50 glass border-b border-slate-200/50 h-20 flex items-center">
      <div className="max-w-7xl mx-auto px-6 w-full flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center gap-10">
          ...
          <Link to="/planos" className={`text-sm font-bold ${location.pathname === '/planos' ? 'text-[#1c2d51]' : 'text-slate-600 hover:text-[#1c2d51]'}`}>Planes</Link>
          <Link to="/demo" className={`text-sm font-bold ${isDemo ? 'text-[#1c2d51]' : 'text-slate-600 hover:text-[#1c2d51]'}`}>Demo</Link>
        </div>

        <div className="flex items-center gap-4">
          <div className="hidden sm:flex items-center gap-4">
            <Link to="/login" className="text-sm font-bold text-slate-700 px-4 hover:text-[#1c2d51]">Iniciar Sesión</Link>
      ...
      {isOpen && (
        <div className="absolute top-20 left-0 right-0 bg-white border-b border-slate-100 p-6 flex flex-col gap-6 animate-in slide-in-from-top duration-300 shadow-xl md:hidden">
          ...
          <Link to="/planos" onClick={() => setIsOpen(false)} className="text-lg font-bold text-[#1c2d51]">Planes</Link>
          <Link to="/demo" onClick={() => setIsOpen(false)} className="text-lg font-bold text-[#1c2d51]">Demo</Link>
          <hr className="border-slate-100" />
          <div className="flex flex-col gap-4">
            <Link to="/login" onClick={() => setIsOpen(false)} className="text-center py-3 font-bold text-slate-600">Iniciar Sesión</Link>
            <Link to="/register" onClick={() => setIsOpen(false)} className="bg-[#1c2d51] text-white py-4 rounded-2xl text-center font-bold">Crear Cuenta</Link>
          </div>
        </div>
      )}
    </nav>
